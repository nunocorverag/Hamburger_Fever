...
class food_element:
    ...
    def draw(self):
        self.image = pygame.image.load(
            "images/food/Final_Sprites/" + self.food_name + ".png"
        ).convert_alpha()
...
class button:
    def __init__(
        self, x, y, image, scale, screen
    ):  # Cuando se usa pygame todo es con imágenes, primero la imágen y luego se importa, incluso el texto.
        width = image.get_width()
        height = image.get_height()
        self.image = pygame.transform.scale(
            image, (int(width * scale), int(height * scale))
        )  # Para pixeles solo se trabaja con float
...
class just_an_image:
    ...
    def draw(self, parallax):
        screen.blit(
            self.image, (self.x - camera_x * parallax, self.y - camera_y * parallax)
        )
...
class speech_bubble:
    ...
    def draw(self, parallax):
        speech_costume = self.ss.imgat(
            (
                self.sheet_x * SPEECH_SPRITES_LENGHT,
                self.sheet_y * SPEECH_SPRITES_HEIGHT,
                SPEECH_SPRITES_LENGHT,
                SPEECH_SPRITES_HEIGHT,
            )
        )
        screen.blit(
            speech_costume, (self.x - camera_x * parallax, self.y - camera_y * parallax)
        )
...
def show_order(requested_order):
    ...
    for i in range(len(requested_order)):
        order_message += (
            str(requested_order[i][0]) + " x " + str(requested_order[i][1]) + "\n"
        )
        ...
        if line:
            if line[0] != "0":
                ...
                screen.blit(
                    ...
                    (
                        x
                        - np.cos(pygame.time.get_ticks() / 100) * 10
                        - camera_x * parallax,
                        y - camera_y * parallax,
                    ),
...
def deliver_order():
    ...
    message = delivered_font.render(
        f"Order delivered {('incorrectly', 'successfully')[message]}",
        True,
        ((255, 255, 228), (156, 185, 59))[message],
    )
...
expanded_background_image = pygame.image.load(
    "images/Fondos/expanded_background_4.png"
).convert_alpha()
expanded_background = just_an_image(expanded_background_image, -100, -280)

speech_sprite_sheet = spritesheet.get_spritesheet(
    "images/other_objects/speech_bubble.png"
)
...
while running:
    ...
    if draw_menu:
        ...
        camera_y += round(
            (camera_central_point_y + camera_central_point_y_extra - camera_y)
            / CAMERA_SPEED
        )
        camera_x += round(
            (camera_central_point_x + camera_central_point_x_extra - camera_x)
            / CAMERA_SPEED
        )
        ...
        if gamestate == 1:
            ...
            if time_left <= 10:
                time_text_color = (217, 36, 60)
            else:
                time_text_color = (119, 214, 193)

            time_left_message = time_left_font.render(
                "Time left:" + str(time_left), True, time_text_color
            )
            time_left_message_white = time_left_font.render(
                "Time left:" + str(time_left), True, (255, 255, 228)
            )
        ...
        for i in range(len(object_order)):
            ...
            if object_order[i].move is True or i == 0:
                object_order[i].yPosition += object_order[i].spdy
                collision_functions.check_collisions(
                    object_order[i], object_order[i - 1], REBOTE, FLOOR_VALUE
                )
        ...
        if len(object_order):
            if object_order[-1].move is False and gamestate == 2:
                gamestate = 3
                show_message = True
                hide_text_order = 0
