...
class the_guy_head:
    ...
    def draw(self, costume):
        if self.pair.show:
            ...
            sum_of_all_y = (
                self.pair.y - self.pair.camera_y + self.offset + self.pair.offset - 204
            )
...
class the_guy:
    ...
    def draw(self):
        if self.show:
            torso_image = pygame.image.load(
                "images/delivery_man/chunky_body.png"
            ).convert_alpha()
            self.screen.blit(
                torso_image,
                (self.x - self.camera_x, self.y - self.camera_y + self.offset),
            )
...
def animation(walk_animation, body, head):
    ...
    head_costume = head_sheet.imgat(
        (
            head.sheet_x * HEAD_SPRITES_LENGHT,
            head.sheet_y * HEAD_SPRITES_HEIGHT,
            HEAD_SPRITES_LENGHT,
            HEAD_SPRITES_HEIGHT,
        )
    )
